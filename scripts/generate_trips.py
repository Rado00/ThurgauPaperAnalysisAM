...
def execute(path):
    df_mz_trips = pd.read_csv(f"{path}\\wege.csv", encoding="latin1")
    df_mz_stages = pd.read_csv(f"{path}\\etappen.csv", encoding="latin1")
    ...
    logging.info("Modes adjusted successfully")
    ...
    logging.info("Passenger trips found successfully")
    ...
    logging.info("Purposes adjusted successfully")
    ...
    logging.info("Trips adjusted successfully")
    ...
    logging.info("Activity durations added successfully")
    ...
    logging.info("Persons filtered successfully")
...
def create_activity_chain(group):
    ...
    logging.info(f"Activity chain created: {chain}")
...
if __name__ == '__main__':
    ...
    # Set up logging
    logging.basicConfig(filename="logs\\generate_trips.log",
                        level=logging.INFO,
    ...
    # TODO check the column name and value
    zone_polygon = gdf[gdf['gemname'] == 'Frauenfeld'].iloc[0]['geometry']
    ...
    filtered_trips = trips[
        trips['origin_point'].apply(lambda point: point.within(zone_polygon)) &
        trips['destination_point'].apply(lambda point: point.within(zone_polygon))
    ]

    # Create activity chains
    df_activity_chains = filtered_trips.groupby(['person_id']).apply(create_activity_chain).reset_index()

    filtered_trips.to_csv(new_path + '\\trips.csv')
    ...
    pio.write_image(fig1, plots_directory + '\\mode_share_distribution_total_counts.png')
    logging.info("Mode share distribution total counts plot saved successfully")
    # TODO uncomment the line below to show the plot
    # fig1.show()
    ...
    pio.write_image(fig2, plots_directory + '\\mode_share_distribution_percentage.png')
    logging.info("Mode share distribution percentage plot saved successfully")
    # TODO uncomment the line below to show the plot
    # fig2.show()
    ...
    pio.write_image(fig, plots_directory + '\\departure_arrival_times_over_a_day.png')
    logging.info("Departure and arrival times over a day plot saved successfully")
    # TODO uncomment the line below to show the plot
    # fig.show()
    ...
    fig1.update_layout(width=600, height=600)
    pio.write_image(fig1, plots_directory + '\\purpose_distribution_total_counts.png')
    logging.info("Purpose distribution total counts plot saved successfully")
    # TODO uncomment the line below to show the plot
    # fig1.show()
    ...
    fig2.update_layout(width=600, height=600)
    pio.write_image(fig2, plots_directory + '\\purpose_distribution_percentage.png')
    logging.info("Purpose distribution percentage plot saved successfully")
    # TODO uncomment the line below to show the plot
    # fig2.show()
    ...
    pio.write_image(fig, plots_directory + '\\activity_chain_distribution_total_counts.png')
    logging.info("Activity chain distribution total counts plot saved successfully")
    # TODO uncomment the line below to show the plot
    # fig.show()
